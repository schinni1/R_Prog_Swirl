...
| Vectors come in two different flavors: atomic vectors and lists. An atomic vector contains exactly one data type, whereas
| a list may contain multiple data types. We'll explore atomic vectors further before we get to lists.
...
| Logical vectors can contain the values TRUE, FALSE, and NA (for 'not available'). These values are generated as the result
| of logical 'conditions'. Let's experiment with some simple conditions.
...
Error: unexpected numeric constant in "num_vect(0.5.55"
> num_vect(0.5,55, -10, 6)
...
> num_vect <- 0.5,55, -10, 6
Error: unexpected ',' in "num_vect <- 0.5,"
> num_vector <-seq((0.5,55, -10, 6))
Error: unexpected ',' in "num_vector <-seq((0.5,"
> num_vector <-seq(0.5,55, -10, 6)
Error in seq.default(0.5, 55, -10, 6) : too many arguments
> ?numeric vector
Error: unexpected symbol in "?numeric vector"
> as.numeric("0.5","55","-10", "6")
[1] 0.5

| Not quite, but you're learning! Try again. Or, type info() for more options.

| Recall that the c() function is used for creating a vector. If you forget how to use it, use ?c to access the help file.
| Don't forget to assign the result to a new variable called num_vect.

> as.numeric(c("0.5","55","-10", "6"))
[1]   0.5  55.0 -10.0   6.0
...
> num_vect <- c("0.5","55","-10", "6", recursive = FALSE)
...
> num_vect <- c("0.5","55","-10", "6", recursive = TRUE)
...
| Remember our lesson on vector arithmetic? The theme was that R performs many operations on an element-by-element basis. We
...
| The first element of num_vect is 0.5, which is less than 1 and therefore the statement 0.5 < 1 is TRUE. The second element
| of num_vect is 55, which is greater than 1, so the statement 55 < 1 is FALSE. The same logic applies for the third and
| fourth elements.
...
| are greater than or equal to 6, the second and fourth elements of the result are TRUE and the first and third elements are
| FALSE.
...
| Try your best to predict the result of each of the following statements. You can use pencil and paper to work them out if
| it's helpful. If you get stuck, just guess and you've got a 50% chance of getting the right answer!
...
| This is a tricky one. Remember that the `!` symbol negates whatever comes after it. There's also an 'order of operations'
| going on here. Conditions that are enclosed within parentheses should be evaluated first. Then, work your way outwards.
...
| Don't worry if you found these to be tricky. They're supposed to be. Working with logical statements in R takes practice,
| but your efforts will be rewarded in future lessons (e.g. subsetting and control structures).
...
| Create a character vector that contains the following words: "My", "name", "is". Remember to enclose each word in its own
| set of double quotes, so that R knows they are character strings. Store the vector in a variable called my_char.

> my_char <- c("My"," "name"", "is")
Error: unexpected symbol in "my_char <- c("My"," "name"
> my_char <- c("My"," "name", "is")
Error: unexpected symbol in "my_char <- c("My"," "name"
> my_char <- c("My","name", "is")
...
| For a slightly more complicated example, we can join two vectors, each of length 3. Use paste() to join the integer vector
| 1:3 with the character vector c("X", "Y", "Z"). This time, use sep = "" to leave no space between the joined elements.

> pase(1:3, c("X", "Y", "Z"), sep = "")
Error: could not find function "pase"
...
> paste(LETTERS, 1:4, sep = "-")
 [1] "A-1" "B-2" "C-3" "D-4" "E-1" "F-2" "G-3" "H-4" "I-1" "J-2" "K-3" "L-4" "M-1" "N-2" "O-3" "P-4" "Q-1" "R-2" "S-3" "T-4"
...
| We'll discuss coercion in another lesson, but all it really means that the numbers 1, 2, 3, and 4 in the output above are
| no longer numbers to R, but rather characters "1", "2", "3", and "4".
